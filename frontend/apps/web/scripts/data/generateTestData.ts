...
import behaviors from '../../src/behaviors.json'

const IN_PATH = path.resolve(__dirname, 'tests.json')
const OUT_PATH = path.resolve(__dirname, '../../src/tests.json')
    ...
    if (t.scenarios) {
      for (const scenario of t.scenarios) {
        if (!scenario.Behaviors) {
          ...
          const randomBehaviors = allBehaviors
            .sort(() => 0.5 - Math.random())
