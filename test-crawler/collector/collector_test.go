...
func TestListGoFilesInFolder(t *testing.T) {
	...
	t.Run("Read golang files from mocks/config directory", func(t *testing.T) {
		system, files, err := ListGoFilesInFolder("../mocks/config", nil)
		assert.NoError(t, err)
		assert.Equal(t, system, "config")
		assert.Equal(t, 2, len(files))
	})

	t.Run("Read golang files from mocks directory", func(t *testing.T) {
		system, files, err := ListGoFilesInFolder("../mocks", nil)
		assert.NoError(t, err)
		assert.Equal(t, system, "mocks")
